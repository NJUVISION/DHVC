...
def safe_load_pretrained(checkpoint_path, net, map_location='cpu'):
    ...
    if hasattr(net, "remap_legacy_state_dict"):
        pretrained_dict = net.remap_legacy_state_dict(dict(pretrained_dict))
...
def evaluate_one_video(args, quality, item_path, net, decoded_frame_dir=None):
    ...
    if decoded_frame_dir is not None:
        save_dir = Path(decoded_frame_dir) / item_name
    ...
    for fi, ori_fp in enumerate(ori_frame_paths[:num_frames]):
        ...
        with torch.no_grad():
            ...
            sum_bpp += _to_float(frame_metrics['bpp'])
            sum_bpp1 += _to_float(frame_metrics['bpp1'])
            sum_bpp2 += _to_float(frame_metrics['bpp2'])
            sum_bpp3 += _to_float(frame_metrics['bpp3'])
            sum_bpp4 += _to_float(frame_metrics['bpp4'])
        ...
        print('Intra frame:', fi, "Progressive Layer 0 ", 'bpp:', _to_float(frame_metrics['bpp']), 'psnr:', psnr)
...
def main():
    ...
    parser.add_argument('--checkpoint', type=Path, default=Path('checkpoint.pth.tar'))
    parser.add_argument('--results-root', type=Path, default=Path('runs/results'))
    ...
    default_dataset_roots = {
        'kodak': Path('/datasets/Kodak/'),
    }

    suffix = 'allframes' if (args.num_frames is None) else f'first{args.num_frames}'
    results_dir = args.results_root / f'{args.test_dataset}-gop{args.gop}-{suffix}'
    ...
    net = dhic()
    safe_load_pretrained(args.checkpoint, net)
    net.to(device).eval()
    logging.info("Model loaded successfully.")

    dataset_path = args.dataset_root or default_dataset_roots.get(args.test_dataset)
    if dataset_path is None:
        logging.error(f"Unknown dataset '{args.test_dataset}'. Pass --dataset-root explicitly.")
        return
