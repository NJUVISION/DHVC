...
from .layers import FeatureExtractor, MyConvNeXtBlock, TemporalLatentBlock2, \
...
class HierarchicalVideoCodec(nn.Module):
        ...
        for _, (block, fea_block) in enumerate(zip(self.dec_blocks, self.fea_blocks)):
            if getattr(block, 'is_latent_block', False):
                ...
                if i <= 4:
                    ...
                    feature, dec_feature, stats = block(feature, dec_feature, f_ctx, enc_feature=f_enc, mode='progressive',
                                                        get_latent=get_latent)
                if get_latent:
    ...
    @torch.no_grad()
    def decompress(self, compressed_object, z_lists, num_latents=4, get_latent=False):
        ...
        for _, (block, fea_block) in enumerate(zip(self.dec_blocks, self.fea_blocks)):
            if getattr(block, 'is_latent_block', False):
                ...
                f_ctx = [z_list[i] for z_list in z_lists]
                # if i == 3:
                #     feature, dec_feature, stats = block(feature, dec_feature, f_ctx, mode='progressive',
                #                                         get_latent=get_latent)
                # else:
...
def dhvc_base(lamb=512):
    ...
    cfg['dec_blocks'] = [
        # 1x1
        *[TemporalLatentBlock2(dec_dims[0], z_dims[0], enc_width=enc_dims[-1], kernel_size=1, mlp_ratio=4) for _ in range(1)],
        ...
        *[TemporalLatentBlock2(dec_dims[1], z_dims[1], enc_width=enc_dims[-2], kernel_size=3, mlp_ratio=3) for _ in range(2)],
        MyConvNeXtBlock(dec_dims[1], kernel_size=3, mlp_ratio=3),
        patch_upsample(dec_dims[1], dec_dims[2], rate=2),
        # 4x4
        MyConvNeXtBlock(dec_dims[2], kernel_size=5, mlp_ratio=2),
        *[TemporalLatentBlock2(dec_dims[2], z_dims[2], enc_width=enc_dims[-3], kernel_size=5, mlp_ratio=2) for _ in range(1)],
        MyConvNeXtBlock(dec_dims[2], kernel_size=5, mlp_ratio=2),
        patch_upsample(dec_dims[2], dec_dims[3], rate=2),
        # 8x8
        MyConvNeXtBlock(dec_dims[3], kernel_size=7, mlp_ratio=1.75),
        *[TemporalLatentBlock2(dec_dims[3], z_dims[3], enc_width=enc_dims[-4], kernel_size=7, mlp_ratio=1.75) for _ in range(1)],
        MyConvNeXtBlock(dec_dims[3], kernel_size=7, mlp_ratio=1.75),
