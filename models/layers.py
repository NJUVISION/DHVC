...
class TemporalLatentBlock(nn.Module):
    def __init__(self, width, zdim, enc_width=None, kernel_size=7, mlp_ratio=2):
        ...
        concat_ch = (width * 2) if (enc_width is None) else (width + enc_width)
        self.resnet_front   = MyConvNeXtBlock(width, kernel_size=kernel_size, mlp_ratio=mlp_ratio)
        self.resnet_front1  = MyConvNeXtBlock(width, kernel_size=kernel_size, mlp_ratio=mlp_ratio)
        self.resnet_front2  = MyConvNeXtBlock(width, kernel_size=kernel_size, mlp_ratio=mlp_ratio)
        self.resnet_front3  = MyConvNeXtBlock(width, kernel_size=kernel_size, mlp_ratio=mlp_ratio)
        self.resnet_ctx     = MyConvNeXtBlock(width, kernel_size=kernel_size, mlp_ratio=mlp_ratio)
        self.resnet_end     = MyConvNeXtBlock(width, kernel_size=kernel_size, mlp_ratio=mlp_ratio)
        self.posterior0 = MyConvNeXtBlock(enc_width, kernel_size=kernel_size)
        self.posterior1 = MyConvNeXtBlock(width, kernel_size=kernel_size)
        self.posterior2 = MyConvNeXtBlock(width, kernel_size=kernel_size)
        self.prior_merge = conv_k1s1(width * 3, width)
        self.post_merge  = conv_k1s1(concat_ch, width)
        self.end_merge   = conv_k1s1(width * 2, width)
        self.posterior   = conv_k3s1(width, zdim)
        self.z_proj      = conv_k1s1(zdim, width)
        self.prior       = conv_k1s1(width, zdim*2)
    ...
    def transform_prior(self, feature, ctx_feature):
        ...
        """
        feature = self.resnet_front(feature)
        ctx_feature1 = self.resnet_front1(ctx_feature[0])
        ctx_feature2 = self.resnet_front2(ctx_feature[1])
        ctx_feature = torch.cat([feature, ctx_feature1, ctx_feature2], dim=1)
        ctx_feature = self.prior_merge(ctx_feature)
        ctx_feature = self.resnet_front3(ctx_feature)
    ...
    def fuse_feature_and_z(self, feature, ctx_feature, z):
        # add the new information carried by z to the feature
        ctx_feature = self.resnet_ctx(ctx_feature)
        merged = torch.cat([feature, ctx_feature], dim=1)
        merged = self.end_merge(merged)
        feature = merged + self.z_proj(z)
        return feature

    def forward(self, feature, ctx_feature=None, enc_feature=None, mode='trainval',
                get_latent=False, strings=None):
        """ a complicated forward function
        ...
        feature = self.fuse_feature_and_z(feature, ctx_feature, z)
        feature = self.resnet_end(feature)
        if get_latent:
...
class TemporalContextPrediction1(nn.Module):
...
class TemporalLatentBlock1(nn.Module):
    def __init__(self, width, zdim, enc_width=None, kernel_size=7, mlp_ratio=2):
        super().__init__()
        self.in_channels  = width
        self.out_channels = width
        ...
        self.tcp = TemporalContextPrediction1(width=width, kernel_size=kernel_size)
    ...
    def forward(self, feature, ctx_feature=None, enc_feature=None, mode='trainval',
        ...
        Args:
            ...
            enc_feature (torch.Tensor): feature map
        """
        feature, pm, pv = self.transform_prior(feature, ctx_feature)
...
class TemporalContextPrediction2(nn.Module):
    def __init__(self, width, kernel_size=7):
        super().__init__()
...
class TemporalLatentBlock2(nn.Module):
    def __init__(self, width, zdim, enc_width=None, kernel_size=7, mlp_ratio=2):
        super().__init__()
        self.in_channels  = width
        self.out_channels = width
        ...
        self.tcp = TemporalContextPrediction2(width=width, kernel_size=kernel_size)
